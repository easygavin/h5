...
define(function (require, exports, module) {
  var page = require('page'),
    ...
    template = require('../../views/f3d/ball.html'),
  ...
  var init = function (data, forward) {
    ...
    // 处理返回
    page.setHistoryState({url: lot + "/ball", data: params},
      lot + "/ball",
      (JSON.stringify(params).length > 2 ? "?data=" + encodeURIComponent(JSON.stringify(params)) : "") + "#" + lot + "/ball",
      canBack ? 1 : 0);
  };
  ...
  var readBuffer = function () {
    // 缓存的数据
    bufferData = util.getLocalJson(util.keyMap.LOCAL_F3D);
  ...
  var addModeItems = function () {
    ...
    var ballModeTpl = require('/tpl/digit/ball_mode');
  ...
  var showOpenNumbers = function () {
    var ballLastTpl = require('/tpl/digit/ball_last');
  ...
  var bindEvent = function () {
    ...
    // 右菜单
    $(document).off(events.touchStart(), ".select").
      on(events.touchStart(), ".select", function (e) {
        events.handleTapEvent(this, this, events.activate(), e);
        return true;
      });

    $(document).off(events.activate(), ".select").
      on(events.activate(), ".select", function (e) {
        $(".popup").show();
        util.showCover();
        return true;
      });
  ...
  var toList = function () {
    ...
    util.setLocalJson(util.keyMap.LOCAL_F3D, bufferData);

    if (bufferData.length === 0) {
      page.toast("请至少选择 1 注");
      return false;
    }
    page.init(lot + "/list", {}, 1);
    return true;
  };
